...
def test_new_creates_expected_dirs(runner, tmp_path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        for d in ["datasources", "entities", "repositories", "services", "utils", "endpoints"]:
...
def test_new_creates_main_py(runner, tmp_path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        assert 'autodiscover("endpoints")' in content
        assert "asgi_app" in content


def test_new_endpoints_init_is_empty(runner, tmp_path):
    with runner.isolated_filesystem(temp_dir=tmp_path):
        runner.invoke(app, ["new", "meu-server"])
        # Discovery moved into autodiscover(); the generated __init__.py is a
        # bare package marker with no import loop.
        assert open("meu-server/endpoints/__init__.py").read().strip() == ""
